...
def create_role(*, code, level, permissions, scope=RoleProfile.Scope.SITE, requires_shift=False):
...
class EmployeeOrganizationAPITests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.unconfigured_role = Group.objects.create(name="Custom Group")
    ...
    def test_employee_list_returns_roles_and_organization_details(self):
        ...
        self.assertEqual(row["user_details"]["role"]["name"], self.secondary_role.name)
        self.assertEqual(
            {role["name"] for role in row["user_details"]["roles"]},
            {self.employee_role.name, self.secondary_role.name},
        )
    ...
    def test_employee_options_returns_user_roles(self):
        ...
        self.assertEqual(row["role"]["name"], self.secondary_role.name)
        self.assertEqual(
    ...
    def test_employee_groups_endpoint_returns_assignable_groups(self):
        ...
        self.assertEqual(groups[self.secondary_role.group.pk]["role"]["name"], self.secondary_role.name)
