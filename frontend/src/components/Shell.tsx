...
export function Shell({ user, children }: { user: UserProfile; children: ReactNode }) {
  const [opened, setOpened] = useState(false); const [search, setSearch] = useState('');
  const { colorScheme, toggleColorScheme } = useMantineColorScheme(); const location = useLocation(); const navigate = useNavigate(); const dark = colorScheme === 'dark';
  const visibleSections = useMemo(() => sections.map((section) => ({ ...section, items: section.items.filter((item) => can(user, item.permission) && (item.to !== '/shift' || user.role?.requires_shift)) })).filter((section) => section.items.length > 0), [user]);
  const allItems = useMemo(() => visibleSections.flatMap((section) => section.items), [visibleSections]);
  const searchMatches = useMemo(() => { const value = search.trim().toLowerCase(); if (!value) return []; return allItems.filter((item) => item.label.toLowerCase().includes(value)).slice(0, 5); }, [allItems, search]);
  async function logout() { try { await api.auth.logout(); } finally { navigate('/login'); } }
  function submitSearch() { if (!searchMatches.length) return; navigate(searchMatches[0].to); setSearch(''); }
  const roleLabel = user.role?.name || (user.is_superuser ? 'Administrator' : 'User'); const siteLabel = user.employee?.site?.name || 'Company-wide';

  return <div className="app-bg"><div className="aurora-orb one" /><div className="aurora-orb two" /><div className="aurora-orb three" /><AppShell padding="lg" navbar={{ width: 238, breakpoint: 'md', collapsed: { mobile: !opened } }} header={{ height: 74 }} styles={{ main: { background: 'transparent' }, header: { background: dark ? 'rgba(8, 13, 25, 0.88)' : 'rgba(255, 255, 255, 0.92)', backdropFilter: 'blur(12px)', borderColor: dark ? 'rgba(148, 163, 184, 0.10)' : 'rgba(15, 23, 42, 0.07)' }, navbar: { background: dark ? 'rgba(8, 13, 25, 0.94)' : 'rgba(255, 255, 255, 0.95)', borderColor: dark ? 'rgba(148, 163, 184, 0.10)' : 'rgba(15, 23, 42, 0.07)' } }}>
    <AppShell.Header><Group h="100%" px="lg" justify="space-between" gap="lg"><Group gap="sm" miw={230}><Burger opened={opened} onClick={() => setOpened((value) => !value)} hiddenFrom="md" size="sm" /><Group gap="sm"><ThemeIcon variant="gradient" gradient={{ from: 'indigo', to: 'cyan', deg: 120 }} size={36} radius="xl">E</ThemeIcon><div><Text fw={850} size="sm" lh={1.1}>ERP Command Center</Text><Text size="xs" c="dimmed" mt={3}>Sales & operations</Text></div></Group></Group><TextInput className="topbar-search" leftSection={<IconSearch size={17} />} rightSection={<Text size="xs" c="dimmed" className="shortcut-hint">⌘ K</Text>} placeholder="Search modules…" value={search} onChange={(event) => setSearch(event.currentTarget.value)} onKeyDown={(event) => { if (event.key === 'Enter') submitSearch(); if (event.key === 'Escape') setSearch(''); }} visibleFrom="sm" styles={{ input: { borderRadius: 999 } }} /><Group gap="xs"><Tooltip label={dark ? 'Use light theme' : 'Use dark theme'}><ActionIcon variant="subtle" radius="xl" size="lg" onClick={() => toggleColorScheme()} aria-label="Toggle color scheme">{dark ? <IconSun size={18} /> : <IconMoon size={18} />}</ActionIcon></Tooltip><Menu shadow="md" width={260} position="bottom-end"><Menu.Target><button className="user-chip" type="button"><Avatar radius="xl" size="sm" color="indigo">{(user.first_name || user.username).slice(0, 1).toUpperCase()}</Avatar><span className="user-chip-copy"><Text size="sm" fw={750}>{user.first_name || user.username}</Text><Text size="xs" c="dimmed">{roleLabel} · {siteLabel}</Text></span><IconChevronDown size={15} /></button></Menu.Target><Menu.Dropdown><Menu.Label>Workspace</Menu.Label><Menu.Item closeMenuOnClick={false} disabled leftSection={<IconBuilding size={16} />} rightSection={user.current_shift ? <Badge color="teal" size="sm">Shift open</Badge> : undefined}>{siteLabel}</Menu.Item><Menu.Divider /><Menu.Label>Account</Menu.Label><Menu.Item leftSection={<IconPower size={16} />} color="red" onClick={logout}>Sign out</Menu.Item></Menu.Dropdown></Menu></Group></Group></AppShell.Header>
    <AppShell.Navbar p="sm"><AppShell.Section grow component={ScrollArea} scrollbarSize={4}><Stack gap="md">{visibleSections.map((section) => <div key={section.label}><Text px="sm" mb={6} size="xs" fw={800} tt="uppercase" c="dimmed" lts="0.08em">{section.label}</Text><Stack gap={2}>{section.items.map((item) => { const active = location.pathname === item.to || (item.to !== '/' && location.pathname.startsWith(item.to)); return <NavLink key={item.to} component={Link} to={item.to} active={active} label={item.label} leftSection={<span className="nav-icon">{item.icon}</span>} onClick={() => setOpened(false)} variant="light" styles={{ root: { minHeight: 40, borderRadius: 10 }, label: { fontWeight: active ? 750 : 600 } }} />; })}</Stack></div>)}</Stack></AppShell.Section><AppShell.Section><Divider my="md" /><Text size="xs" c="dimmed" px="sm">ERP workspace</Text></AppShell.Section></AppShell.Navbar>
