  ...
  return (
    <Stack gap="xl">
      <Group justify="space-between" align="flex-end" wrap="wrap">
        <div><Text size="sm" c="indigo.3" fw={800}>MASTER DATA</Text><Title order={1} mt={4}>{title}</Title><Text c="dimmed" mt={4}>{subtitle}</Text></div>
        <Button leftSection={<IconPlus size={17} />} onClick={openCreate} variant="gradient" gradient={{ from: 'indigo', to: 'cyan', deg: 120 }}>New</Button>
      </Group>

      <Paper className="glass" p="md" radius="lg" withBorder>
        <Group>
          <TextInput flex={1} leftSection={<IconSearch size={16} />} placeholder={searchPlaceholder} value={search} onChange={(event) => setSearch(event.currentTarget.value)} onKeyDown={(event) => { if (event.key === 'Enter') { setPage(1); void load(1); } }} />
          <Button variant="light" onClick={() => { setPage(1); void load(1); }}>Search</Button>
        </Group>
      </Paper>
      ...
      <Paper className="glass bento-card" radius="lg" withBorder>
        <ScrollArea>
          <Table horizontalSpacing="lg" verticalSpacing="sm" highlightOnHover striped={false} miw={720}>
            <Table.Thead><Table.Tr>{columns.map((column) => <Table.Th key={column.key}>{column.label}</Table.Th>)}<Table.Th ta="right">Actions</Table.Th></Table.Tr></Table.Thead>
            <Table.Tbody>
              {data.results.map((row) => (
                <Table.Tr key={String(row.id)}>
                  {columns.map((column) => <Table.Td key={column.key}>{column.render ? column.render(row[column.key], row) : display(row[column.key])}</Table.Td>)}
                  <Table.Td>
              ...
              {!loading && !data.results.length && <Table.Tr><Table.Td colSpan={columns.length + 1}><Text c="dimmed" ta="center" py="xl">No records found.</Text></Table.Td></Table.Tr>}
              {loading && <Table.Tr><Table.Td colSpan={columns.length + 1}><Text c="dimmed" ta="center" py="xl">Loading…</Text></Table.Td></Table.Tr>}
            </Table.Tbody>
          </Table>
        </ScrollArea>
          ...
          <Stack>
            {fields.map((field) => {
              if (field.type === 'boolean') {
                return <Checkbox key={field.key} label={field.label} checked={Boolean(form[field.key])} onChange={(event) => setForm((current) => ({ ...current, [field.key]: event.currentTarget.checked }))} />;
              }
