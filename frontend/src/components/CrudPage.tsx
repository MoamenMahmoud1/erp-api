...
export function CrudPage({ title, subtitle, fields, columns, list, create, update, remove, canEdit = true, canDelete = true, searchPlaceholder = 'Search' }: Props) {
  ...
  return (
    <Stack gap="xl">
      <Group justify="space-between" align="flex-end" wrap="wrap"><div><Text size="sm" c="indigo.3" fw={800}>MASTER DATA</Text><Title order={1} mt={4}>{title}</Title><Text c="dimmed" mt={4}>{subtitle}</Text></div><Button leftSection={<IconPlus size={17} />} onClick={openCreate} variant="gradient" gradient={{ from: 'indigo', to: 'cyan', deg: 120 }}>New</Button></Group>
