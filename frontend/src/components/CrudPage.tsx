  ...
  return (
    <Stack gap="xl">
      <Group justify="space-between" align="flex-end" wrap="wrap">
        <div><Text size="xs" c="indigo.3" fw={800} tt="uppercase" lts=".08em">Master data</Text><Title order={1} mt={5}>{title}</Title><Text c="dimmed" mt={4}>{subtitle}</Text></div>
        {create && <Button leftSection={<IconPlus size={17} />} onClick={openCreate} variant="gradient" gradient={{ from: 'indigo', to: 'cyan', deg: 120 }} radius="lg">New {itemTitle}</Button>}
      </Group>
      <Paper className="glass" p="sm" radius="xl" withBorder>
        <Group gap="sm" wrap="nowrap">
          <TextInput flex={1} radius="lg" leftSection={<IconSearch size={16} />} placeholder={searchPlaceholder} value={search} onChange={(event) => setSearch(event.currentTarget.value)} onKeyDown={(event) => { if (event.key === 'Enter') { setPage(1); void load(1); } }} />
          <Button radius="lg" variant="light" onClick={() => { setPage(1); void load(1); }}>Search</Button>
        </Group>
      </Paper>
      <Paper className="surface-panel bento-card" radius="xl" withBorder>
        <ScrollArea>
          <Table horizontalSpacing="lg" verticalSpacing="sm" highlightOnHover striped={false} miw={720}>
            ...
            <Table.Tbody>
              {data.results.map((row) => <Table.Tr key={String(row.id)}>{columns.map((column) => <Table.Td key={column.key}>{column.render ? column.render(row[column.key], row) : display(row[column.key])}</Table.Td>)}{hasWrite && <Table.Td><Group justify="flex-end" gap={4}>{canEdit && update && <ActionIcon radius="xl" variant="subtle" aria-label={`Edit ${itemTitle}`} onClick={() => openEdit(row)}><IconEdit size={17} /></ActionIcon>}{canDelete && remove && <ActionIcon radius="xl" variant="subtle" color="red" aria-label={`Delete ${itemTitle}`} onClick={() => void onDelete(Number(row.id))}><IconTrash size={17} /></ActionIcon>}</Group></Table.Td>}</Table.Tr>)}
      ...
      <Modal opened={modalOpen} onClose={() => setModalOpen(false)} title={editing ? `Edit ${itemTitle}` : `New ${itemTitle}`} centered size="md" radius="xl" overlayProps={{ backgroundOpacity: 0.28, blur: 0 }}>
        <form onSubmit={submit}><Stack gap="md">
          {visibleFields.map((field) => {
            ...
            if (field.type === 'number') return <NumberInput key={field.key} label={field.label} value={form[field.key] as number | string} onChange={(value) => setForm((current) => ({ ...current, [field.key]: value }))} required={field.required} radius="lg" />;
            if (field.type === 'select') return <Select key={field.key} label={field.label} data={field.options || []} value={String(form[field.key] ?? '')} onChange={(value) => setForm((current) => ({ ...current, [field.key]: value }))} required={field.required} allowDeselect={field.clearable === true} clearable={field.clearable === true} radius="lg" />;
            return <TextInput key={field.key} label={field.label} value={String(form[field.key] ?? '')} onChange={(event) => setForm((current) => ({ ...current, [field.key]: event.currentTarget.value }))} required={field.required} radius="lg" />;
          })}
          <Button type="submit" fullWidth radius="lg" disabled={!create && !editing}>{editing ? 'Save changes' : `Create ${itemTitle}`}</Button>
        </Stack></form>
      </Modal>
    </Stack>
  );
