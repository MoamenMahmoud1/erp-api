...
export function InventoryLocationsPage() {
  return <RecordsPage eyebrow="INVENTORY" title="Locations" subtitle="Sales vehicles, warehouses and active stock locations." list={api.inventory.locations} columns={[{ key: 'id', label: '#' }, { key: 'name', label: 'Location' }, { key: 'location_type', label: 'Type' }, { key: 'employee', label: 'Employee' }, { key: 'is_active', label: 'Status' }, { key: 'created_at', label: 'Created' }]} />;
}

export function InventoryMovementsPage() {
  return <RecordsPage eyebrow="INVENTORY" title="Movements" subtitle="Immutable stock movement history." list={api.inventory.movements} columns={[{ key: 'id', label: '#' }, { key: 'movement_type', label: 'Type' }, { key: 'source_location', label: 'From' }, { key: 'destination_location', label: 'To' }, { key: 'reference', label: 'Reference' }, { key: 'created_at', label: 'Date' }]} />;
}
...
export function InventoryTransferPage() {
  ...
  return <Stack gap="xl"><div><Text size="sm" c="indigo.3" fw={800}>INVENTORY</Text><Title order={1} mt={4}>Transfer stock</Title><Text c="dimmed" mt={4}>Move inventory between two locations with one atomic operation.</Text></div><Card className="glass bento-card" radius="lg" withBorder p="xl" maw={820}><form onSubmit={submit}><Stack><Select label="Source location" data={locations.results.map((item) => ({ value: String(item.id), label: `${item.name || item.id} · ${item.location_type || 'location'}` }))} value={source} onChange={(value) => setSource(value || '')} searchable required /><Select label="Destination location" data={locations.results.map((item) => ({ value: String(item.id), label: `${item.name || item.id} · ${item.location_type || 'location'}` }))} value={destination} onChange={(value) => setDestination(value || '')} searchable required /><Select label="Product" data={products.results.map((item) => ({ value: String(item.id), label: `${item.name || item.id} · stock ${item.stock_quantity ?? '—'}` }))} value={product} onChange={(value) => setProduct(value || '')} searchable required /><NumberInput label="Quantity" min={1} value={quantity} onChange={setQuantity} required /><TextInput label="Reference" value={reference} onChange={(event) => setReference(event.currentTarget.value)} placeholder="Optional" /><Button type="submit" loading={loading} variant="gradient" gradient={{ from: 'indigo', to: 'cyan', deg: 120 }}>Transfer stock</Button></Stack></form></Card></Stack>;
}
