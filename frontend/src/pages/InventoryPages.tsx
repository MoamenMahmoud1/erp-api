...
export function InventoryLocationsPage() {
  return <RecordsPage eyebrow="INVENTORY" title="Locations" subtitle="Sales vehicles and warehouses organized by branch/store." list={api.inventory.locations} columns={[{ key: 'id', label: '#' }, { key: 'site_name', label: 'Branch / store' }, { key: 'name', label: 'Location' }, { key: 'location_type', label: 'Type', format: (value) => <Badge variant="light">{String(value).replaceAll('_', ' ')}</Badge> }, { key: 'employee_name', label: 'Assigned employee' }, { key: 'is_active', label: 'Status' }, { key: 'created_at', label: 'Created' }]} />;
}

export function InventoryMovementsPage() {
  return <RecordsPage eyebrow="INVENTORY" title="Movements" subtitle="Stock transfers and movements across branches, stores and locations." list={api.inventory.movements} columns={[{ key: 'id', label: '#' }, { key: 'movement_type', label: 'Type' }, { key: 'source_site_name', label: 'From branch' }, { key: 'source_location_name', label: 'From location' }, { key: 'destination_site_name', label: 'To branch' }, { key: 'destination_location_name', label: 'To location' }, { key: 'created_by_name', label: 'Created by' }, { key: 'reference', label: 'Reference' }, { key: 'created_at', label: 'Date' }]} />;
}
...
export function InventoryTransferPage() {
  ...
  const locationOptions = locations.results.map((item) => ({ value: String(item.id), label: `${item.site_name || 'Unassigned site'} · ${item.name || 'Unnamed location'} · ${String(item.location_type || 'location').replaceAll('_', ' ')}` }));
