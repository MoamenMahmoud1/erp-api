...
const journalStatusOptions = [{ value: 'draft', label: 'Draft' }, { value: 'posted', label: 'Posted' }];
const journalSourceOptions = [
  { value: 'manual', label: 'Manual' },
  { value: 'invoice.sale', label: 'Invoice sale' },
  { value: 'invoice.return', label: 'Invoice return' },
  { value: 'invoice.sale.reversal', label: 'Invoice sale reversal' },
  { value: 'purchase.confirmation', label: 'Purchase confirmation' },
  { value: 'purchase.return', label: 'Purchase return' },
  { value: 'purchase.confirmation.reversal', label: 'Purchase reversal' },
  { value: 'payment.collection', label: 'Payment collection' },
  { value: 'payment.refund', label: 'Payment refund' },
  { value: 'payment.supplier', label: 'Supplier payment' },
  { value: 'payment.transfer.approval', label: 'Transfer approval' },
];
...
export function JournalsPage({ user }: { user: UserProfile }) {
  return <RecordsPage eyebrow="ACCOUNTING" title="Journal entries" subtitle="Posted and draft accounting entries generated by ERP operations." list={api.accounting.journalEntries}
    filters={[
      { key: 'entry_date_from', label: 'From date', type: 'date' },
      { key: 'entry_date_to', label: 'To date', type: 'date' },
      { key: 'status', label: 'Status', type: 'select', options: journalStatusOptions },
      { key: 'created_by_name', label: 'Created by', type: 'text', placeholder: 'Name or username' },
      { key: 'posted_by_name', label: 'Posted by', type: 'text', placeholder: 'Name or username' },
      { key: 'source_type', label: 'Source', type: 'select', options: journalSourceOptions },
    ]} details={{ load: api.accounting.journalEntry, render: (data) => <JournalDetails data={data} /> }} columns={[{ key: 'number', label: '#' }, { key: 'entry_date', label: 'Date' }, { key: 'description', label: 'Description' }, { key: 'reference', label: 'Reference' }, { key: 'status', label: 'Status', format: (value) => <Badge color={String(value) === 'posted' ? 'teal' : 'yellow'} variant="light">{String(value)}</Badge> }]} actions={[{ label: 'Post', color: 'teal', visible: (row) => can(user, 'accounting.post_journal_entry') && String(row.status) === 'draft', run: (row) => api.accounting.postJournalEntry(Number(row.id)) }]} />;
}
