...
export function StatementsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const [pnl, bs, cf] = await Promise.all([api.accounting.profitAndLoss(params), api.accounting.balanceSheet(to), api.accounting.cashFlow(params)]);
...
export function BalancesPage() {
  return <RecordsPage eyebrow="ACCOUNTING" title="AR / AP balances" subtitle="Customer and supplier balances from the operational ledger." list={async (queryString) => { const [customers, suppliers] = await Promise.all([api.accounting.customerBalances(queryString), api.accounting.supplierBalances(queryString)]); const customerRows = Array.isArray(customers) ? customers : []; const supplierRows = Array.isArray(suppliers) ? suppliers : []; return { count: customerRows.length + supplierRows.length, next: null, previous: null, results: [...customerRows.map((row: Record<string, unknown>) => ({ ...row, party_type: 'Customer', party_name: row.customer_name })), ...supplierRows.map((row: Record<string, unknown>) => ({ ...row, party_type: 'Supplier', party_name: row.supplier_name }))] }; }} columns={[{ key: 'party_type', label: 'Type' }, { key: 'party_name', label: 'Party' }, { key: 'invoiced', label: 'Invoiced' }, { key: 'purchased', label: 'Purchased' }, { key: 'paid', label: 'Paid' }, { key: 'balance', label: 'Balance' }]} />;
}
