...
export function ExpensesPage() {
  return <CrudPage title="Expenses" subtitle="Record operating expenses against a payment account." list={api.accounting.expenses} create={api.accounting.createExpense} canEdit={false} canDelete={false} fields={[{ key: 'expense_account', label: 'Expense account ID', type: 'number', required: true }, { key: 'payment_account', label: 'Payment account ID', type: 'number', required: true }, { key: 'amount', label: 'Amount', type: 'number', required: true }, { key: 'expense_date', label: 'Date', required: true }, { key: 'description', label: 'Description', required: true }, { key: 'reference', label: 'Reference' }]} columns={[{ key: 'id', label: '#' }, { key: 'expense_date', label: 'Date' }, { key: 'description', label: 'Description' }, { key: 'amount', label: 'Amount' }, { key: 'expense_account', label: 'Expense account' }, { key: 'payment_account', label: 'Payment account' }]} />;
}
...
function PeriodWorkspace() {
  ...
  async function create() {
    ...
    try { await api.accounting.createPeriod({ name, start_date: start, end_date: end }); setModal(false); setName(''); setStart(null); setEnd(null); await load(); notifications.show({ title: 'Period created', message: 'The accounting period is open.', color: 'teal' }); }
  ...
  return <Stack gap="xl"><Group justify="space-between" align="flex-end"><div><Text size="sm" c="indigo.3" fw={800}>ACCOUNTING</Text><Title order={1} mt={4}>Periods</Title><Text c="dimmed" mt={4}>Control which accounting dates are open for posting.</Text></div><Button onClick={() => setModal(true)} variant="gradient" gradient={{ from: 'indigo', to: 'cyan', deg: 120 }}>New period</Button></Group><RecordsPage eyebrow="ACCOUNTING" title="Open and closed periods" subtitle="Period locking is enforced by the accounting posting service." list={async () => data} columns={[{ key: 'name', label: 'Period' }, { key: 'start_date', label: 'Start' }, { key: 'end_date', label: 'End' }, { key: 'is_closed', label: 'Closed' }, { key: 'closed_at', label: 'Closed at' }]} actions={[{ label: 'Close', color: 'red', visible: (row) => !row.is_closed, run: (row) => api.accounting.closePeriod(Number(row.id)) }]} /><Modal opened={modal} onClose={() => setModal(false)} title="New accounting period" centered><Stack><TextInput label="Name" value={name} onChange={(event) => setName(event.currentTarget.value)} required /><DatePickerInput label="Start date" value={start} onChange={setStart} required /><DatePickerInput label="End date" value={end} onChange={setEnd} required /><Button onClick={() => void create()}>Create period</Button></Stack></Modal></Stack>;
}
...
export function TrialBalancePage() {
  ...
  return <Stack gap="xl"><div><Text size="sm" c="indigo.3" fw={800}>ACCOUNTING</Text><Title order={1} mt={4}>Trial balance</Title><Text c="dimmed" mt={4}>A compact control report that verifies total debits equal total credits.</Text></div><Card className="glass" withBorder radius="lg"><Group align="flex-end"><DatePickerInput label="From" value={from} onChange={setFrom} clearable /><DatePickerInput label="To" value={to} onChange={setTo} clearable /><Button loading={loading} onClick={() => void load()}>Run report</Button></Group></Card>{data && <SimpleGrid cols={{ base: 1, md: 3 }}><Card className="glass" withBorder><Text c="dimmed" size="xs">TOTAL DEBIT</Text><Text size="xl" fw={900}>{String(data.total_debit)}</Text></Card><Card className="glass" withBorder><Text c="dimmed" size="xs">TOTAL CREDIT</Text><Text size="xl" fw={900}>{String(data.total_credit)}</Text></Card><Card className="glass" withBorder><Badge color={data.balanced ? 'teal' : 'red'} size="lg">{data.balanced ? 'Balanced' : 'Unbalanced'}</Badge></Card></SimpleGrid>}{data && <RecordsPage eyebrow="CONTROL REPORT" title="Account totals" subtitle="Grouped posted journal activity by account." list={async () => ({ count: rows.length, next: null, previous: null, results: rows })} columns={[{ key: 'account__code', label: 'Code' }, { key: 'account__name', label: 'Account' }, { key: 'account__account_type', label: 'Type' }, { key: 'debit', label: 'Debit' }, { key: 'credit', label: 'Credit' }, { key: 'balance', label: 'Balance' }]} />}</Stack>;
}
