...
export function ExpensesPage() {
  ...
  const accountOptions = useMemo<CrudOption[]>(() => (accounts?.results || []).map((account) => ({
  ...
  return (
    <CrudPage
      ...
      canDelete={false}
      fields={[
        { key: 'expense_account', label: 'Expense account', type: 'select', options: accountOptions, required: true },
        { key: 'payment_account', label: 'Payment account', type: 'select', options: accountOptions, required: true },
      ...
      columns={[
        ...
        { key: 'amount', label: 'Amount' },
        {
          ...
          render: (value) => accountOptions.find((option) => option.value === String(value ?? ''))?.label || '—',
        },
        {
          key: 'payment_account',
...
function PeriodWorkspace() {
  ...
  async function create() {
    ...
    try {
      ...
      setModal(false); setName(''); setStart(null); setEnd(null);
      await load();
