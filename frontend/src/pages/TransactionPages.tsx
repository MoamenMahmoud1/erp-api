...
export function CreatePurchasePage({ user }: { user: UserProfile }) {
  ...
  const [lines, setLines] = useState<Line[]>([{ product: '', quantity: 1, unit_purchase_price: '', batch_number: '', manufactured_date: '', expiry_date: '' }]);
  const [loading, setLoading] = useState(false);
  function updateLine(index: number, patch: Partial<Line>) { setLines((current) => current.map((line, lineIndex) => lineIndex === index ? { ...line, ...patch } : line)); }
  async function submit(event: FormEvent) {
    event.preventDefault();
    const invalid = lines.some((line) => !line.product || Number(line.quantity) < 1 || line.unit_purchase_price === '' || Number(line.unit_purchase_price) < 0 || (line.manufactured_date && line.expiry_date && line.expiry_date < line.manufactured_date));
    if (!supplier || (canChooseSite && !site) || invalid) return;
    setLoading(true);
    try {
      await api.purchases.create({
        supplier: Number(supplier),
        ...(site ? { site: Number(site) } : {}),
        reference,
        items: lines.map((line) => ({
          product: Number(line.product),
          quantity: Number(line.quantity),
          unit_purchase_price: Number(line.unit_purchase_price),
          ...(line.batch_number?.trim() ? { batch_number: line.batch_number.trim() } : {}),
          ...(line.manufactured_date ? { manufactured_date: line.manufactured_date } : {}),
      ...
      setReference(''); setLines([{ product: '', quantity: 1, unit_purchase_price: '', batch_number: '', manufactured_date: '', expiry_date: '' }]);
  ...
  return <Stack gap="xl">
    <WorkflowHeader eyebrow="PURCHASING" title="New purchase" subtitle={canChooseSite ? 'Receive stock with optional lot and expiry traceability.' : `Create a purchase for ${user.employee?.site?.name || 'your store'}.`} />
    <Card className="glass" withBorder radius="lg" p="xl" maw={1160}>
      <form onSubmit={submit}>
        <Stack>
          <SimpleGrid cols={{ base: 1, sm: canChooseSite ? 2 : 1 }}>
            <Select label="Supplier" searchable data={supplierOptions} value={supplier} onChange={(value) => setSupplier(value || '')} required />
            {canChooseSite && <Select label="Branch / store" searchable data={siteOptions} value={site} onChange={(value) => setSite(value || '')} required />}
          </SimpleGrid>
          <TextInput label="Reference" value={reference} onChange={(event) => setReference(event.currentTarget.value)} />
          <Divider my="sm" />
          <Group justify="space-between" align="center"><div><Text fw={800}>Received items</Text><Text size="xs" c="dimmed">Record the supplier lot and dates when available. Expired batches will not be selected for normal stock issuance.</Text></div></Group>
          {lines.map((line, index) => (
            <Card key={index} withBorder radius="lg" p="md" bg="transparent">
              <Stack gap="md">
                <SimpleGrid cols={{ base: 1, sm: 3 }}>
                  <Select label={`Product ${index + 1}`} searchable data={productOptions} value={line.product} onChange={(value) => updateLine(index, { product: value || '' })} required />
                  <NumberInput label="Quantity" min={1} value={line.quantity} onChange={(value) => updateLine(index, { quantity: value })} required />
                  <NumberInput label="Unit purchase price" min={0} value={line.unit_purchase_price} onChange={(value) => updateLine(index, { unit_purchase_price: value })} required />
                </SimpleGrid>
                <SimpleGrid cols={{ base: 1, sm: 3 }}>
                  <TextInput label="Batch / lot number" placeholder="e.g. LOT-2026-09-A" value={line.batch_number} onChange={(event) => updateLine(index, { batch_number: event.currentTarget.value })} />
                  <TextInput label="Manufactured date" type="date" value={line.manufactured_date} onChange={(event) => updateLine(index, { manufactured_date: event.currentTarget.value })} />
                  <TextInput label="Expiry date" type="date" value={line.expiry_date} min={line.manufactured_date || undefined} onChange={(event) => updateLine(index, { expiry_date: event.currentTarget.value })} />
                </SimpleGrid>
                <Group justify="flex-end"><Button type="button" variant="subtle" color="red" disabled={lines.length === 1} onClick={() => setLines((current) => current.filter((_, lineIndex) => lineIndex !== index))}>Remove item</Button></Group>
              </Stack>
            </Card>
          ))}
          <Group justify="space-between" wrap="wrap"><Button type="button" variant="light" onClick={() => setLines((current) => [...current, { product: '', quantity: 1, unit_purchase_price: '', batch_number: '', manufactured_date: '', expiry_date: '' }])}>Add item</Button><Button type="submit" loading={loading} variant="gradient" gradient={{ from: 'violet', to: 'indigo', deg: 120 }}>Create draft purchase</Button></Group>
        </Stack>
      </form>
    </Card>
  </Stack>;
...
export function CollectPaymentPage() {
  ...
  const [customer, setCustomer] = useState(''); const [cash, setCash] = useState<number | string>(0); const [transfer, setTransfer] = useState<number | string>(0); const [loading, setLoading] = useState(false);
...
export function SupplierPaymentPage() {
  ...
  const [supplier, setSupplier] = useState(''); const [cash, setCash] = useState<number | string>(0); const [transfer, setTransfer] = useState<number | string>(0); const [reference, setReference] = useState(''); const [loading, setLoading] = useState(false);
