...
def collect(*, customer, cash_amount, transfer_amount, collected_by_id, actor=None):
    ...
    for invoice in invoices:
        due = quantize_money(invoice.total - paid_map.get(invoice.pk, Decimal("0")))
        if due > 0:
    ...
    log_operation("payment.collection", user=collected_by_id, customer=customer.pk, invoices_allocated=len(outstanding))
    return payment
