...
class AccountingReturnTests(TestCase):
    def setUp(self):
        ...
        self.user = CustomUserModel.objects.create_user(email="returns@test.local", password="strong-password-123")
    ...
    def test_sales_return_reverses_revenue_and_cogs(self):
        ...
        return_item = InvoiceReturnItem.objects.create(invoice_return=returned, invoice_item=item, quantity=1, unit_price=Decimal("100.00"))
    ...
    def test_purchase_return_reduces_inventory_and_ap(self):
        purchase = Purchase.objects.create(supplier_id=1, created_by=self.user, reference="P-1")
