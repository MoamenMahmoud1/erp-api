...
class AccountingAutomationTests(TestCase):
    ...
    def test_sales_posting_creates_ar_revenue_and_cogs(self):
        ...
        self.assertEqual(
            entry.lines.filter(account__code="1200", debit=Decimal("200.00")).count(),
            1,
        )
        self.assertEqual(
            entry.lines.filter(account__code="4100", credit=Decimal("200.00")).count(),
            1,
        )
        self.assertEqual(
            entry.lines.filter(account__code="5100", debit=Decimal("80.00")).count(),
            1,
        )
        self.assertEqual(
            entry.lines.filter(account__code="1100", credit=Decimal("80.00")).count(),
            1,
        )
        self.assertEqual(
            post_sales_invoice(invoice=invoice, actor_id=self.user.pk, company=self.company).pk,
            entry.pk,
        )
    ...
    def test_purchase_posting_creates_inventory_and_ap(self):
        ...
        PurchaseItem.objects.create(
            ...
            product=self.product,
            quantity=3,
            unit_purchase_price=Decimal("40.00"),
        )

        entry = post_purchase(purchase=purchase, actor_id=self.user.pk, company=self.company)
        self.assertEqual(entry.status, JournalEntry.Status.POSTED)
        self.assertEqual(
            entry.lines.filter(account__code="1100", debit=Decimal("120.00")).count(),
            1,
        )
        self.assertEqual(
            entry.lines.filter(account__code="2100", credit=Decimal("120.00")).count(),
            1,
        )
    ...
    def test_collection_posts_cash_and_ar(self):
        ...
        self.assertEqual(entry.status, JournalEntry.Status.POSTED)
        self.assertEqual(
            entry.lines.filter(account__code="1010", debit=Decimal("60.00")).count(),
            1,
        )
        self.assertEqual(
            entry.lines.filter(account__code="1200", credit=Decimal("60.00")).count(),
            1,
        )
    ...
    def test_default_accounts_are_created_once(self):
        ...
        self.assertEqual(Account.objects.filter(company=self.company).count(), 8)
