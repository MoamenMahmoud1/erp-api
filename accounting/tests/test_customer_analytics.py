...
class CustomerAnalyticsTests(TestCase):
    ...
    def _invoice(self, customer, quantity, price, discount=Decimal("0")):
    ...
    def test_ranking_uses_net_invoice_total_after_coupon_discount(self):
        discounted = self._invoice(self.top_customer, 10, "25", discount=Decimal("220"))
        self._invoice(self.bottom_customer, 5, "25")

        top, bottom = customer_sales_ranking()

        self.assertEqual(discounted.total, Decimal("30"))
        self.assertEqual(top[0]["customer_id"], self.bottom_customer.pk)
        self.assertEqual(top[0]["revenue"], Decimal("125"))
        self.assertEqual(bottom[0]["customer_id"], self.top_customer.pk)
        self.assertEqual(bottom[0]["revenue"], Decimal("30"))
