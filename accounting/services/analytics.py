...
def inventory_dashboard(*, low_stock_threshold=10, site_id=None):
    ...
    return {
        ...
        "expired_units": sum((row.quantity for row in expired_rows), 0),
        "expiring_7_days_units": sum((row.quantity for row in expiring_7_rows), 0),
        "expiring_30_days_units": sum((row.quantity for row in expiring_30_rows), 0),
        "expired_batch_count": len(expired_rows),
        "expiring_7_days_batch_count": len(expiring_7_rows),
        "expiring_30_days_batch_count": len(expiring_30_rows),
...
def customer_sales_ranking(*, date_from=None, date_to=None, limit=5, site_id=None):
    ...
    line_total = ExpressionWrapper(F("unit_price") * F("quantity"), output_field=DecimalField(max_digits=18, decimal_places=2))
    sales_rows = list(
        items.values("invoice__customer_id", "invoice__customer__name")
        .annotate(
            quantity=Coalesce(Sum("quantity"), 0),
            gross_revenue=Coalesce(Sum(line_total), ZERO),
            invoice_count=Count("invoice", distinct=True),
        )
        .order_by("invoice__customer_id")
    )
    return_queryset = InvoiceReturnItem.objects.filter(invoice_return__invoice__status__in=SALES_STATUSES)
    if site_id is not None:
        return_queryset = return_queryset.filter(invoice_return__site_id=site_id)
    return_queryset = _range_filter(return_queryset, "invoice_return__created_at", date_from, date_to)
    return_rows = return_queryset.values("invoice_item__invoice__customer_id").annotate(returns=Coalesce(Sum("invoice_return__refund_amount"), ZERO))
    return_map = {row["invoice_item__invoice__customer_id"]: row["returns"] for row in return_rows}
...
def dashboard_overview(*, date_from=None, date_to=None, site_id=None):
    ...
    top_customers, bottom_customers = customer_sales_ranking(
        date_from=date_from,
        date_to=date_to,
        limit=5,
        site_id=site_id,
    ...
    return {
        "counts": company_master_data_counts(),
        "site_id": site_id,
        "sales": sales_dashboard(date_from=date_from, date_to=date_to, site_id=site_id),
        "purchases": purchase_dashboard(date_from=date_from, date_to=date_to, site_id=site_id),
        "inventory": inventory_dashboard(site_id=site_id),
        ...
        "top_products": top_products(date_from=date_from, date_to=date_to, limit=6, site_id=site_id),
        "sales_by_employee": sales_by_employee(date_from=date_from, date_to=date_to, site_id=site_id),
