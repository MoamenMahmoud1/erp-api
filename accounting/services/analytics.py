...
def inventory_dashboard(*, low_stock_threshold=10, site_id=None):
    ...
    stock_rows = products.values("id", "name").annotate(stock=Coalesce(Sum("stock_balances__quantity"), 0), inventory_value=Coalesce(Sum("stock_balances__total_cost"), ZERO)).order_by("name")
    ...
    return {"site_id": site_id, "total_units": total_units, "inventory_value": inventory_value, "product_count": len(list(stock_rows)), "low_stock_threshold": low_stock_threshold, "low_stock": low_stock, "low_stock_count": len(low_stock)}
